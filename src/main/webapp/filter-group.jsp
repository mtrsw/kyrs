...
<head>
    <meta charset="UTF-8">
    <title>Фильтр по группе</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body style="background:linear-gradient(135deg,#f5f7fa,#e4e9f2);min-height:100vh;">
<%@ include file="navbar.jsp" %>
<div class="container py-5">
    <h2 class="text-center mb-4">Фильтрация по группе</h2>
    <div class="row justify-content-center mb-4">
        <div class="col-md-6">
            <form action="filter-group" method="get" class="d-flex gap-2">
                <select name="groupId" class="form-select" required>
                    <option value="">— выберите группу —</option>
                    <c:forEach var="g" items="${groups}">
                        <option value="${g.id}" ${selectedGroup == g.id ? 'selected' : ''}>${g.groupNumber}</option>
                    </c:forEach>
                </select>
